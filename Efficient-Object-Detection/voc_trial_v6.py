...
load_pkl_file = tmp_path + "voc_annotations_multiscale.pkl"
...
train_loss  = voc_path + "voc_losses_v6.csv"
...
print("Fit model on training data (" +\
      str(len(voc_object_list[1])) + " training samples).")
img_disp = voc_object_list[1][25][0]
img_dims = voc_object_list[1][25][1]
img_box  = tf.cast(
    tf.sparse.to_dense(tf.sparse.reorder(
        voc_object_list[1][25][2])), tf.float32)
...
train(voc_model, sub_batch, batch_size, 
      voc_object_list, training_loss, img_disp, img_box, 
      st_step, max_steps, optimizer, checkpoint, ck_manager, 
      label_dict, init_lr=init_lr, decay=decay_rate, 
      step_cool=step_cool, display_step=display_step, img_scale=img_scale, 
      disp_rows=img_dims[0], disp_cols=img_dims[1], disp_scale=disp_scale, 
