...
def gaussian_dist_2d(
    ...
    gauss_xy = gauss_x * gauss_y * (gauss_c**2)
    return gauss_xy
...
def format_data(
    ...
    for n_label in range(len(tmp_sorted)):
        ...
        tmp_std  = tf.maximum(
            1.0, sigma*tf.math.sqrt(tmp_area))
        ...
        if (tmp_y_upp - tmp_y_low) > 0 \
            ...
            tmp_center = gaussian_dist_2d(
                grid_x, grid_y, mu_x=new_x_cen, 
                mu_y=new_y_cen, stddev=tmp_std)
            ...
            new_y_cen  = int(0.5*(tmp_y_low + tmp_y_upp))
            tmp_center = gaussian_dist_1d(
                tmp_y_coord, mu_x=new_y_cen, stddev=tmp_std)
            ...
            new_x_cen  = int(0.5*(tmp_x_low + tmp_x_upp))
            tmp_center = gaussian_dist_1d(
                tmp_x_coord, mu_x=new_x_cen, stddev=tmp_std)
