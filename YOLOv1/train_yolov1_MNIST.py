...
from tf_yolov1 import build_model, format_data, model_loss
...
def show_heatmap(
    image, img_labels, center=False, img_rows=384, img_cols=384):
    ...
    if center:
        ...
        tmp_array = max_probs
    tmp_object = cen_output
    
    tmp_array = tf.expand_dims(tmp_array, axis=2)
    tmp_heatmap = tf.image.resize(
        tf.expand_dims(tmp_array, axis=0), [img_w, img_h])
    tmp_heatmap = tf.squeeze(tmp_heatmap, axis=3)
    
    tmp_object = tf.expand_dims(tmp_object, axis=2)
    tmp_center = tf.image.resize(
        tf.expand_dims(tmp_object, axis=0), [img_w, img_h])
    tmp_center = tf.squeeze(tmp_center, axis=3)
    
    fig, ax = plt.subplots(1)
    tmp_img = np.array(image, dtype=np.uint8)
    ax.imshow(tmp_img)
    tmp = ax.imshow(tmp_heatmap[0], "jet", alpha=0.50)
    fig.colorbar(tmp, ax=ax)
    
    fig.suptitle("Ground Truth Heatmap")
    fig.savefig("true_heatmap.jpg", dpi=199)
    plt.close()
    del fig, ax
    ...
    tmp_img = np.array(image, dtype=np.uint8)
    ax.imshow(tmp_img)
    tmp = ax.imshow(tmp_center[0], "jet", alpha=0.50)
    fig.colorbar(tmp, ax=ax)
    
    fig.suptitle("Ground Truth Center Map")
    fig.savefig("true_center_map.jpg", dpi=199)
    plt.close()
    del fig, ax
    return None
...
def train(
    ...
    for step in range(st_step, max_steps):
        ...
        # Loss is computed per image. #
        acc_losses = 0.0
        reg_losses = 0.0
        cls_losses = 0.0
        cen_losses = 0.0
        for tmp_idx in batch_sample:
            image, bbox, class_id, img_dim = \
            ...
            tmp_labels = format_data(
                label, img_dim, num_classes, img_pad=img_pad)
            tmp_image  = 127.5 * (image[0] + 1.0)
            show_heatmap(tmp_image, tmp_labels, 
                         img_rows=img_pad[0], img_cols=img_pad[1])
            
            with tf.GradientTape() as grad_tape:
                tmp_output = model(image, training=True)
                tmp_losses = \
                    model_loss(tmp_labels, tmp_output)
                all_losses = \
...
train_loss = voc_path + "mnist_losses_yolo_mobilenetv2.csv"
...
train(
    mnist_dataset, training_loss, 
    fcos_model, batch_size, model_optimizer, 
    checkpoint, ck_manager, st_step, max_steps, 
    init_lr=init_lr, decay_step=decay_step, 
    display_step=disp_step, step_cool=step_cool, 
    step_save=save_step, save_loss_file=train_loss)
