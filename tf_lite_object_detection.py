...
def build_model(
    ...
    img_rows=448, img_cols=448, seperable=True, batch_norm=True):
    # No FPN. #
    ...
    x_blk1_out = cnn_block(
        x_input, n_filters, 3, 3, 1, 2, "block_1", 
        seperable=seperable, batch_norm=batch_norm)
    
    feature_shp = \
        [batch_sz, int(img_rows/2), int(img_cols/2), 12]
    x_feature_1 = tf.concat([
        x_blk1_out, tf.reshape(x_input, feature_shp)], axis=3)
    
    # Block 2. #
    x_blk2_out = cnn_block(
        x_feature_1, 2*n_filters, 3, 3, 1, 2, 
        "block_2", seperable=seperable, batch_norm=batch_norm)
    
    feature_shp = \
        [batch_sz, int(img_rows/4), int(img_cols/4), 4*n_filters]
    x_feature_2 = tf.concat([
        x_blk2_out, tf.reshape(x_blk1_out, feature_shp)], axis=3)
    
    # Block 3. #
    x_blk3_out = cnn_block(
        x_feature_2, 4*n_filters, 3, 3, 1, 2, 
    ...
    x_blk4_out = cnn_block(
        x_feature_3, 8*n_filters, 3, 3, 1, 2, 
    ...
    x_blk5_out  = cnn_block(
        x_feature_4, 16*n_filters, 3, 3, 1, 2, 
    ...
    x_blk6_out = cnn_block(
        x_feature_5, 32*n_filters, 3, 3, 1, 2, 
