...
train_loss = \
    model_path + "coco_losses_retinanet_attn_resnet50.csv"
ckpt_model = model_path + "coco_retinanet_attn_resnet50"
restore_flag = False
...
anchor_sizes = [24.0, 48.0, 96.0, 192.0, 384.0]
anchor_scales = [2**x for x in [-1/3, 0.0, 1/3, 2/3]]

retinanet_model = retinanet_attn_module.RetinaNetAttn(
    num_classes, label_2_id, img_dims, 
    n_layers=n_layers, n_heads=n_heads, 
    d_model=d_model, d_ffwd=d_ffwd, anchor_sizes=anchor_sizes, 
    anchor_scales=anchor_scales, backbone_model="resnet50")
model_optimizer = tf.optimizers.SGD(momentum=0.9)

print("-" * 50)
print("RetinaNet Model Built (" +\
      str(retinanet_model.n_anchors) + " anchors).")
