...
def compute_iou(boxes1, boxes2):
    ...
    """
    boxes1_corners = convert_to_corners(
        tf.cast(boxes1, tf.float32))
    boxes2_corners = convert_to_corners(
        tf.cast(boxes2, tf.float32))
    
    lu = tf.maximum(
        boxes1_corners[:, None, :2], boxes2_corners[:, :2])
    rd = tf.minimum(
        boxes1_corners[:, None, 2:], boxes2_corners[:, 2:])
...
def visualize_detections(
    ...
    figsize=(7, 7), linewidth=1, color=[0, 0, 1]):
    ...
    fig.savefig("detect.jpg", dpi=199)
    return None
