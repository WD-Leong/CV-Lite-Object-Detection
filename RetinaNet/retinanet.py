...
class FeaturePyramid(keras.layers.Layer):
    ...
    def call(self, images, training=False):
        c3_output, c4_output, c5_output = \
            self.backbone(images, training=training)
...
class RetinaNet(keras.Model):
    ...
    def __init__(self, num_classes, backbone=None, **kwargs):
        ...
        self.cls_head = build_head(
            9*num_classes, prior_probability)
    ...
    def call(self, image, training=False):
        ...
        for feature in features:
            ...
            cls_outputs.append(tf.reshape(
                self.cls_head(feature), [N, -1, self.num_classes]))
...
class DecodePredictions(tf.keras.layers.Layer):
    ...
    def call(self, images, predictions):
        image_shape  = tf.cast(tf.shape(images), dtype=tf.float64)
        anchor_boxes = \
...
class RetinaNetClassificationLoss(tf.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        p_t  = tf.where(
            tf.equal(y_true, 1.0), probs, 1.0 - probs)
        loss = alpha * tf.pow(1.0 - p_t, self._gamma) * cross_entropy
        return tf.reduce_sum(loss, axis=-1)

class RetinaNetLoss(tf.losses.Loss):
    """Wrapper to combine both the losses"""
    def __init__(
        self, num_classes=80, alpha=0.25, gamma=2.0, delta=1.0):
        super(RetinaNetLoss, self).__init__(
            reduction="auto", name="RetinaNetLoss")
        self._clf_loss = \
            RetinaNetClassificationLoss(alpha, gamma)
    ...
    def call(self, y_true, y_pred):
        ...
        cls_labels = tf.one_hot(
            ...
            depth=self._num_classes, dtype=tf.float32)
        ...
        clf_loss = tf.where(
            tf.equal(ignore_mask, 1.0), 0.0, clf_loss)
        box_loss = tf.where(
            tf.equal(positive_mask, 1.0), box_loss, 0.0)
