...
def build_model(
    num_classes, img_dims, n_layers=3, 
    n_heads=4, d_model=256, d_ffwd=1024, 
    n_anchors=9, backbone_model="resnet50"):
    """
    ...
    def format_data(
        ...
        for n_level in range(len(self.box_areas)):
            ...
            for n_anchor in range(self.n_anchors):
                ...
                new_anchors = np.zeros_like(tmp_anchors)
                new_anchors[:, :2] = tmp_anchors[:, :2] * stride
                new_anchors[:, 2:] = tmp_anchors[:, 2:]
                ...
                for n_box in range(len(gt_boxes)):
                    ...
                    num_target = np.sum(
                        tmp_thresh.numpy().astype(np.int))
                    
                    if num_target > 0:
                        anchor_valid = tmp_anchors[tmp_thresh, :]
                        
                        # Assign the ground truth. #
                        for n_valid in range(num_target):
                            tmp_anchor = anchor_valid[n_valid]
                            
                            tmp_bbox  = gt_boxes[n_box, :]
                            tmp_label = 4 + int(tmp_bbox[4])
                            
                            y_pos = int(tmp_anchor[0])
                            x_pos = int(tmp_anchor[1])
                            
                            tmp_y_off = y_pos * stride - tmp_bbox[0]
                            tmp_y_off = tmp_y_off / tmp_anchor[2]
                            tmp_x_off = x_pos * stride - tmp_bbox[1]
                            tmp_x_off = tmp_x_off / tmp_anchor[3]
                            
                            tmp_h_scale = tmp_bbox[2] / tmp_anchor[2]
                            tmp_w_scale = tmp_bbox[3] / tmp_anchor[3]
                            
                            # Bounding Box Regression Outputs. #
                            box_reg = [
                                tmp_y_off, tmp_x_off, 
                                tmp_h_scale, tmp_w_scale]
                            
                            # Assign the ground truth labels to #
                            # the output array for training.    #
                            tmp_output[y_pos, x_pos, :4] = box_reg
                            tmp_output[
                                y_pos, x_pos, tmp_label] = 1.0
