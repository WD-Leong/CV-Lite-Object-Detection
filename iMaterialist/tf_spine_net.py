...
def build_model(
    n_filters, n_classes, tmp_pi=0.99, n_repeats=2, 
    seperable=True, batch_norm=True, norm_order="norm_first"):
    ...
    x_features = tf.concat(
        [x_enc1_features, 
         x_enc2_features, x_blk3_out, 
         x_dec1_features, x_dec2_features, 
         x_dec3_out, x_dec4_features], axis=3)
...
def obj_detect_results(
    ...
    heatmap=True, transpose=False, 
    thresh=0.50, img_box=None, img_title=None, 
    ...
    if transpose:
        tmp_output = \
            tf.transpose(tmp_output, [0, 2, 1, 3, 4])
