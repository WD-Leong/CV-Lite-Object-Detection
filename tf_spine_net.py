...
def cnn_block(
    x_cnn_input, n_filters, ker_sz, 
    stride, blk_name, n_repeats=1, 
    ...
    for n_repeat in range(n_repeats):
        bn_name  = blk_name + "_bn_" + str(n_repeat)
        ...
        if batch_norm:
            tmp_bnorm = \
                layers.BatchNormalization(name=bn_name)(tmp_output)
...
def downsample_block(
    x_cnn_input, n_filters, ker_sz, 
    blk_name, seperable=True, batch_norm=True):
    if seperable:
        tmp_output = layers.SeparableConv2D(
            n_filters, (ker_sz, ker_sz), 
            strides=(2,2), padding="same", 
            activation=None, name=blk_name)(x_cnn_input)
    else:
        tmp_output = layers.Conv2D(
            n_filters, (ker_sz, ker_sz), 
            strides=(2,2), padding="same", 
...
def build_model(
    ...
    x_cnn1_out = cnn_block(
        x_input, n_filters, 3, 1, 
        "cnn_block_1", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    
    x_blk1_in  = x_cnn1_out
    x_blk1_out = downsample_block(
        x_cnn1_out, 2*n_filters, 3, "down_block_1", 
        seperable=seperable, batch_norm=batch_norm)
    
    # Block 2. #
    x_cnn2_out = cnn_block(
        x_blk1_out, 2*n_filters, 3, 1, 
        "cnn_block_2", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_blk2_in  = x_blk1_out + x_cnn2_out
    x_blk2_out = downsample_block(
        x_blk2_in, 4*n_filters, 3, "down_block_2", 
        seperable=seperable, batch_norm=batch_norm)
    
    # Block 3. #
    x_cnn3_out = cnn_block(
        x_blk2_out, 4*n_filters, 3, 1, 
        "cnn_block_3", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_blk3_in  = x_blk2_out + x_cnn3_out
    x_blk3_out = downsample_block(
        x_blk3_in, 8*n_filters, 3, "down_block_3", 
        seperable=seperable, batch_norm=batch_norm)
    
    # Block 4. #
    x_cnn4_out = cnn_block(
        x_blk3_out, 8*n_filters, 3, 1, 
        "cnn_block_4", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_blk4_in  = x_blk3_out + x_cnn4_out
    x_blk4_out = downsample_block(
        x_blk4_in, 16*n_filters, 3, "down_block_4", 
        seperable=seperable, batch_norm=batch_norm)
    
    # Block 5. #
    x_cnn5_out = cnn_block(
        x_blk4_out, 16*n_filters, 3, 1, 
        "cnn_block_5", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_blk5_in  = x_blk4_out + x_cnn5_out
    x_blk5_out = downsample_block(
        x_blk5_in, 32*n_filters, 3, "down_block_5", 
        seperable=seperable, batch_norm=batch_norm)
    
    # Block 6. #
    x_cnn6_out = cnn_block(
        x_blk5_out, 32*n_filters, 3, 1, 
        "cnn_block_6", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_blk6_in  = x_blk5_out + x_cnn6_out
    x_blk6_out = downsample_block(
        x_blk6_in, 64*n_filters, 3, "down_block_6", 
        seperable=seperable, batch_norm=batch_norm)
    ...
    x_dec1_out = cnn_block(
        x_ups1_out, 32*n_filters, 3, 1, 
        "dec_block_1", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    ...
    x_dec2_out = cnn_block(
        x_ups2_out, 16*n_filters, 3, 1, 
        "dec_block_2", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
    ...
    x_dec3_out = cnn_block(
        x_ups3_out, 8*n_filters, 3, 1, 
        "dec_block_3", n_repeats=n_repeats, 
        seperable=seperable, batch_norm=batch_norm)
