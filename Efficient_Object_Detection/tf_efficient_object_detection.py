...
def build_model(
    ...
    # Block 1. #
    x_blk1_out = cnn_block(
        x_input, n_filters, 3, 3, 1, 2, 
        "block_1", seperable=seperable, batch_norm=batch_norm)
    
    # Block 2. #
    x_blk2_out = cnn_block(
        x_blk1_out, 2*n_filters, 3, 3, 1, 2, 
        "block_2", seperable=seperable, batch_norm=batch_norm)
    
    # Block 3. #
    x_blk3_out  = cnn_block(
        x_blk2_out, 4*n_filters, 3, 3, 1, 2, 
        "block_3", seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_avg_out_2 = layers.AveragePooling2D()(x_blk2_out)
    x_cnn_out_3 = layers.Conv2D(
        2*n_filters, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="cnn_out_3")(x_blk3_out)
    x_cnn_small = layers.ReLU(
        name="cnn_small_relu")(x_avg_out_2 + x_cnn_out_3)
    
    # Block 4. #
    x_blk4_out  = cnn_block(
        x_blk3_out, 8*n_filters, 3, 3, 1, 2, 
        "block_4", seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_avg_out_3 = layers.AveragePooling2D()(x_blk3_out)
    ...
    x_cnn_medium = layers.ReLU(
        name="cnn_medium_relu")(x_avg_out_3 + x_cnn_out_4)
    
    # Block 5. #
    x_blk5_out  = cnn_block(
        x_blk4_out, 16*n_filters, 3, 3, 1, 2, 
        "block_5", seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_avg_out_4 = layers.AveragePooling2D()(x_blk4_out)
    x_cnn_out_5 = layers.Conv2D(
        8*n_filters, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="cnn_out_5")(x_blk5_out)
    x_cnn_large = layers.ReLU(
        name="cnn_large_relu")(x_avg_out_4 + x_cnn_out_5)
    
    # Block 6. #
    x_blk6_out  = cnn_block(
        x_blk5_out, 32*n_filters, 3, 3, 1, 2, 
        "block_6", seperable=seperable, batch_norm=batch_norm)
    
    # Residual layer. #
    x_avg_out_5 = layers.AveragePooling2D()(x_blk5_out)
    x_cnn_out_6 = layers.Conv2D(
        16*n_filters, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="cnn_out_6")(x_blk6_out)
    x_cnn_relu_6 = layers.ReLU(
        name="cnn_6_relu")(x_avg_out_5 + x_cnn_out_6)
    
    # Upsample network for last layer. #
    x_upsample_1 = layers.UpSampling2D(
        interpolation="bilinear")(x_cnn_relu_6)
    x_ups_cnn_1  = layers.Conv2D(
        8*n_filters, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="ups_cnn_1")(x_upsample_1)
    x_ups_relu_1 = layers.ReLU(name="ups_relu_1")(x_ups_cnn_1)
    
    x_upsample_2 = layers.UpSampling2D(
        interpolation="bilinear")(x_ups_relu_1)
    x_ups_cnn_2  = layers.Conv2D(
        4*n_filters, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="ups_cnn_4")(x_upsample_2)
    x_ups_relu_2 = layers.ReLU(name="ups_relu_4")(x_ups_cnn_2)
    
    x_upsample_3 =layers.UpSampling2D(
        interpolation="bilinear")(x_ups_relu_2)
    x_ups_cnn_3  = layers.Conv2D(
        2*n_filters, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="ups_cnn_5")(x_upsample_3)
    x_cnn_vlarge = layers.ReLU(
        name="cnn_vlarge_relu")(x_ups_cnn_3)
    
    # Feature Pyramid Network. #
    x_fpn_large  = x_ups_relu_1 + x_cnn_large
    ...
    x_fpn_relu_1 = layers.ReLU(name="fpn_relu_1")(x_fpn_cnn_1)
    ...
    x_fpn_relu_2 = layers.ReLU(name="fpn_relu_2")(x_fpn_cnn_2)
    
    x_fpn_medium = x_fpn_relu_1 + x_cnn_medium
    ...
    x_fpn_relu_3 = layers.ReLU(name="fpn_relu_3")(x_fpn_cnn_3)
    
    x_features = tf.concat(
        [x_cnn_small, x_fpn_relu_3, 
         x_fpn_relu_2, x_cnn_vlarge], axis=3)
    
    # 4 scales with 4 coordinates = 16 filters for regression head. #
    x_cnn_reg = layers.Conv2D(
        16, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="cnn_reg")(x_features)
    x_cnn_cls = layers.Conv2D(
        4*n_classes, (3, 3), strides=(1, 1), padding="same", 
        activation="linear", name="x_cls")(x_features)
    
    # Concatenate the outputs. #
    batch_size = tf.shape(x_input)[0]
    reg_heads = tf.nn.sigmoid(tf.reshape(
        x_cnn_reg, [batch_size, img_w, img_h, 4, 4]))
    cls_heads = tf.reshape(
        x_cnn_cls + b_focal, 
        [batch_size, img_w, img_h, 4, n_classes])
