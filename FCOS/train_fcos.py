...
def show_heatmap(
    ...
    for n_layer in range(len(img_labels)):
        ...
        cen_probs = tf.image.resize(
            tf.expand_dims(tmp_object, axis=0), [img_w, img_h])
        cen_probs = tf.squeeze(cen_probs, axis=3)
        
        tmp_center.append(cen_probs)
        tmp_heatmap.append(obj_probs)
    
    tmp_heatmap = tf.concat(tmp_heatmap, axis=0)
    tmp_heatmap = tf.reduce_max(tmp_heatmap, axis=0)
    
    tmp_center = tf.concat(tmp_center, axis=0)
    tmp_center = tf.reduce_max(tmp_center, axis=0)
    ...
    tmp_img = np.array(image, dtype=np.uint8)
    ax.imshow(tmp_img)
    tmp = ax.imshow(tmp_center, "jet", alpha=0.50)
    fig.colorbar(tmp, ax=ax)
...
def train(
    ...
    for step in range(st_step, max_steps):
        step_lr = max(init_lr * np.power(
            decay_rate, int(step / decay_step)), min_lr)
        ...
        l2_params_reg = tf.reduce_sum([
            tf.math.sqrt(tf.reduce_sum(
                tf.nn.l2_loss(var))) for var in model_params])
...
#                         img_rows=img_pad[0], img_cols=img_pad[1])
            ...
            with tf.GradientTape() as grad_tape:
                ...
                tmp_losses = model_loss(
                    tmp_labels, tmp_output, strides, 
                    cls_lambda=1.0, reg_type="l1")
                tot_losses = \
                    tmp_losses[0] + tmp_losses[1] + tmp_losses[2]
                
                if weight_decay > 0.0:
                    all_losses = \
                        tot_losses + weight_decay*l2_params_reg
                else:
                    all_losses = tot_losses
...
model_optimizer = tf.optimizers.SGD(
    learning_rate=init_lr, momentum=0.9)
