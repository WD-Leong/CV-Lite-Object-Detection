...
def show_heatmap(
    ...
    for n_layer in range(len(img_labels)):
        ...
        cen_probs  = tf.image.resize(tf.expand_dims(
            tmp_object, axis=0), [img_w, img_h])
    ...
    tmp_detect = tf.image.combined_non_max_suppression(
        ...
        max_total_size=100, iou_threshold=0.10, score_threshold=0.75)
    ...
    fig.savefig(save_file + "center_map.jpg", dpi=199)
    plt.close("all")
    del fig, ax
    return None
    ...
    for step in range(st_step, max_steps):
        ...
        for tmp_idx in batch_sample:
            ...
            tmp_labels, n_labels = format_data(
                ...
                img_pad=img_pad, b_dim=tmp_sizes)
            ...
            with tf.GradientTape() as grad_tape:
                ...
                tmp_losses = model_loss(tmp_labels, tmp_output)
                tot_losses = \
