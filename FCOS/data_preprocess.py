...
def resize_and_pad_image(
    ...
    return image_padded, image_shape, ratio

def preprocess_data(sample, img_dims=384):
    """
    ...
    # Add the offset to the xy-coordinates. #
#    img_shape = tf.shape(image)
#    h_off_norm = \
#        pad_offset[1] / tf.cast(img_shape[1], tf.float32)
#    w_off_norm = \
#        pad_offset[0] / tf.cast(img_shape[0], tf.float32)
#    bbox[:, 0] = bbox[:, 0] + h_off_norm.numpy()
#    bbox[:, 1] = bbox[:, 1] + w_off_norm.numpy()
    return image, tf.constant(bbox), class_id, img_dims
