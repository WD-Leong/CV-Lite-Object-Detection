...
def format_data(
    ...
    img_pad=None, b_dim=None, strides=None):
    """
    ...
    for na in range(len(strides)):
        ...
        if len(tmp_idx) == 0:
            ...
            for n_label in range(len(tmp_sorted)):
                ...
                tmp_y_cen = int(tmp_label[0] * h_ratio + 0.5)
                tmp_x_cen = int(tmp_label[1] * w_ratio + 0.5)
                idx_class = 5 + int(tmp_label[4])
                
                # Set bounding box coordinates. #
                pos_offsets = [-1, 0, 1]
                tmp_y_coord = [
                    (tmp_y_cen-y_off) for y_off in \
                    pos_offsets if (tmp_y_cen-y_off) >= 0]
                tmp_x_coord = [
                    (tmp_x_cen-x_off) for x_off in \
                    pos_offsets if (tmp_x_cen-x_off) >= 0]
                
                for tmp_x in tmp_x_coord:
                    for tmp_y in tmp_y_coord:
                        if tmp_y >= h_max or tmp_x >= w_max:
                            continue
                        
                        y_off = tmp_y_cen - tmp_y
                        x_off = tmp_x_cen - tmp_x
                        if y_off == 0 and x_off == 0:
                            cen_score = 1.0
                        elif abs(y_off) == 1 and abs(x_off) == 1:
                            cen_score = 0.25
                        else:
                            cen_score = 0.50
                        
                        # Overwrite if center score is higher. #
                        prev_score = tmp_output[tmp_y, tmp_x, 4]
                        if cen_score >= prev_score:
                            tmp_output[tmp_y, tmp_x, 4] = cen_score
                        
                        box_offsets = [
                            tmp_y + 0.5 - tmp_coord[0]/stride, 
                            tmp_coord[2]/stride - tmp_y - 0.5, 
                            tmp_x + 0.5 - tmp_coord[1]/stride, 
                            tmp_coord[3]/stride - tmp_x - 0.5]
                        
                        tmp_output[tmp_y, tmp_x, :4] = box_offsets
                        tmp_output[
                            tmp_y, tmp_x, idx_class] = 1.0
