...
def format_data(gt_labels, img_dim, num_classes, 
    ...
    for na in range(len(strides)):
        ...
        tmp_output = np.zeros([
            int(img_dim[0] / stride), 
            int(img_dim[1] / stride), num_classes+5])
        ...
        if len(tmp_idx) == 0:
            ...
            for n_label in range(len(tmp_sorted)):
                ...
                tmp_y_low = tmp_y_cen - int(tmp_label[2] / 2 * h_ratio)
                tmp_x_low = tmp_x_cen - int(tmp_label[3] / 2 * w_ratio)
                tmp_y_upp = tmp_y_cen + int(tmp_label[2] / 2 * h_ratio)
                tmp_x_upp = tmp_x_cen + int(tmp_label[3] / 2 * w_ratio)
                ...
                if (tmp_y_upp - tmp_y_low) > 0 \
                    ...
                    tmp_y_coord = [
                        float(z) for z in range(tmp_y_low, tmp_y_upp)]
                    tmp_x_coord = [
                        float(z) for z in range(tmp_x_low, tmp_x_upp)]
                    ...
                    tmp_output[
                        tmp_y_low:tmp_y_upp, tmp_x_cen, 2] = \
                            np.maximum(0, tmp_x_cen - tmp_coord[1]/stride)
                    tmp_output[
                        tmp_y_low:tmp_y_upp, tmp_x_cen, 3] = \
                            np.maximum(0, tmp_coord[3]/stride - tmp_x_cen)
                    ...
                    tmp_output[
                        tmp_y_cen, tmp_x_low:tmp_x_upp, 0] = \
                            np.maximum(0, tmp_y_cen - tmp_coord[0]/stride)
                    tmp_output[
                        tmp_y_cen, tmp_x_low:tmp_x_upp, 1] = \
                            np.maximum(0, tmp_coord[2]/stride - tmp_y_cen)
                    ...
                    tmp_output[
                        tmp_y_cen, tmp_x_cen, 0] = \
                            np.maximum(0, tmp_y_cen - tmp_coord[0]/stride)
                    tmp_output[
                        tmp_y_cen, tmp_x_cen, 1] = \
                            np.maximum(0, tmp_coord[2]/stride - tmp_y_cen)
                    
                    tmp_output[
                        tmp_y_cen, tmp_x_cen, 2] = \
                            np.maximum(0, tmp_x_cen - tmp_coord[1]/stride)
                    tmp_output[
                        tmp_y_cen, tmp_x_cen, 3] = \
                            np.maximum(0, tmp_coord[3]/stride - tmp_x_cen)
